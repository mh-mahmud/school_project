//<!--
var url_prefix = 'http://23.88.99.242/~cyhs79edu/'; /*should be start and end with / (slash)*/
...
init = function(){
	...
	switch(page){
		...
		case 'page_create_subject' : page_create_subject();break;
		case 'page_assign_student_subject' : page_assign_student_subject();break;
...
page_assign_student_subject = function() {
...
page_admin_student = function() {
	...
	$("#section_list").on("change", function() {
		...
		// data load for roll no
		$.ajax({
			type: "POST",
			url: url_prefix + 'section_wise_roll_no',
			data: {
				section_id: section_id,
				csrf_webspice_tkn: token
			},
...
page_create_subject = function() {
	$("#has_parent").click(function() {
		var loka = $('#has_parent').prop('checked');
		if(loka == true) {
			$("div#subject_list").css({
				"display": "block"
			});
		}
		else if(loka==false) {
