...
class Parent_controller extends CI_Controller {
	...
	**********************************************/
	...
	public function section_wise_roll_no() {
		$section_id = $this->input->post('section_id');

		$roll_no = $this->db->query("SELECT * FROM student_data WHERE SECTION_ID='".$section_id."' ORDER BY ROLL_NO DESC LIMIT 1")->row();
		if(count($roll_no)) {
	...
	public function class_wise_student_list() {
		...
		$class_id = $this->input->post('class_id');
		$select_sql= "SELECT SI.NAME, SD.STUDENT_DATA_ID, SD.ROLL_NO, SD.CLASS_ID, SD.STUDENT_ID FROM student_info AS SI INNER JOIN student_data AS SD ON SD.STUDENT_ID = SI.STUDENT_ID WHERE SD.CLASS_ID=?";
		$section_list = $this->db->query($select_sql,array($class_id))->result();
		...
		if(	$section_list	)
		{
			foreach($section_list as $list)
			{
				$option_menu.='<option value='.$list->STUDENT_DATA_ID.'>'.$list->NAME. ' (' . $list->ROLL_NO . ')' .'</option>';
			}
