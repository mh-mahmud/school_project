	...
	*/
	...
	public function add_person($data=null) {
		...
		if( $input->person_id ){
			...
			$this->webspice->process_image_single('images',$input->person_id, 'person_full');
			$this->webspice->message_board('Record has been updated!');
			$this->webspice->log_me('product_updated - '.$this->webspice->get_user_id()); # log activities
	...
	public function add_cultural_activities($data=null) {

		$url_prefix = $this->webspice->settings()->site_url_prefix;
		$this->webspice->user_verify($url_prefix.'login', $url_prefix.'add_cultural_activities');
		$this->webspice->permission_verify('add_cultural_activities');
		if( !isset($data['edit']) ){
			$data['edit'] = array(
				'CULTURAL_ACTIVITIES_ID'=>null,
				'TITLE'=>null,
				'DETAILS'=>null,
				'IMAGES'=>null
			);
		...
		if( !$this->form_validation->run() ){
			$this->load->view('admin/add_cultural_activities', $data);
			return FALSE;
		}
		...
		# get input post
		$input = $this->webspice->get_input('cultural_activities_id');
		// dd($input);
		
		#duplicate test
		// $this->webspice->db_field_duplicate_test("SELECT * FROM cultural_activities WHERE PERSON_TYPE=? AND NAME=?", array($input->person_type, $input->name), 'You are not allowed to enter duplicate person', 'PERSON_ID', $input->cultural_activities_id, $data, 'admin/create_product');
		...
		# verify file type
		if( $_FILES['images']['tmp_name'] ){
			$this->webspice->check_file_type(array('jpg','jpeg'), 'images', $data, 'admin/add_cultural_activities');
		}

		# update process
		if( $input->cultural_activities_id ){

			$sql = "
			UPDATE cultural_activities SET TITLE=?, DETAILS=?, IMAGES=?, UPDATED_BY=?,UPDATED_DATE=?
			WHERE CULTURAL_ACTIVITIES_ID=?";
			$this->db->query($sql, array($input->title, $input->details, $_FILES['images']['name'], $this->webspice->get_user_id(), $this->webspice->now(), $input->cultural_activities_id));
			$this->webspice->process_image_single('images',$input->cultural_activities_id, 'cultural_full');
			$this->webspice->message_board('Record has been updated!');
			$this->webspice->log_me('product_updated - '.$this->webspice->get_user_id()); # log activities
			$this->webspice->force_redirect($url_prefix.'manage_cultural_activities');
			return false;
		}
		...
		(TITLE, DETAILS, IMAGES, CREATED_BY,CREATED_DATE,STATUS)
		VALUES
		( ?, ?, ?, ?, ?, 7 )";
		$this->db->query($sql, array($input->title, $input->details, $_FILES['images']['name'], $this->webspice->get_user_id(),$this->webspice->now()));
		$this->webspice->process_image_single('images',$this->db->insert_id(), 'cultural_full');
		...
		$this->webspice->message_board('Record inserted successfully!');
		if($this->webspice->permission_verify('manage_cultural_activities',TRUE)){
			$this->webspice->force_redirect($url_prefix . 'manage_cultural_activities');
			return FALSE;
		}
		$this->webspice->force_redirect($url_prefix.'add_cultural_activities');

	}

	// manage Cultural Activities
	public function manage_cultural_activities() {
		$url_prefix = $this->webspice->settings()->site_url_prefix;
		$this->load->database();
		$orderby = 'ORDER BY cultural_activities.CULTURAL_ACTIVITIES_ID ASC';
		$groupby = null;
		$where = '';
		$page_index = 0;
		...
		$initialSQL = "
		SELECT  * FROM cultural_activities	";


		# filtering records
		if( $this->input->post('filter') ){
			$result = $this->webspice->filter_generator(
				$TableName = 'cultural_activities',
				$InputField = array(),
				$Keyword = array('NAME'),
				$AdditionalWhere = null,
				$DateBetween = null
		...
		switch ($criteria) {
			...
			case 'edit':
				$this->webspice->edit_generator($TableName='cultural_activities', $KeyField='CULTURAL_ACTIVITIES_ID', $key, $RedirectController='admin_controller', $RedirectFunction='add_cultural_activities', $PermissionName='manage_cultural_activities', $StatusCheck=null, $Log='edit_person');
				return false;
				break;
			case 'update':
				$id = $this->uri->segment(3);
				...
				break;
			case 'inactive':
				$this->webspice->action_executer($TableName='cultural_activities', $KeyField='CULTURAL_ACTIVITIES_ID', $key, $RedirectURL='manage_cultural_activities', $PermissionName='manage_cultural_activities', $StatusCheck=7, $ChangeStatus=-7, $RemoveCache='cultural_activities', $Log='inactive_person');
				return false;
				break;

			case 'active':
				$this->webspice->action_executer($TableName='cultural_activities', $KeyField='CULTURAL_ACTIVITIES_ID', $key, $RedirectURL='manage_cultural_activities', $PermissionName='manage_cultural_activities', $StatusCheck=-7, $ChangeStatus=7, $RemoveCache='cultural_activities', $Log='active_person');
				return false;
				break;

			case 'delete':
				$id = $this->webspice->encrypt_decrypt($key, 'decrypt');
				$sql = $this->db->query("DELETE FROM cultural_activities WHERE CULTURAL_ACTIVITIES_ID='".$id."' LIMIT 1");
				if(!unlink($this->webspice->get_path('cultural_full').$id.'.jpg')) {
					die($this->webspice->get_path('cultural_full').$id.'.jpg');
				}
				if($sql) {
					$this->webspice->force_redirect($url_prefix.'manage_cultural_activities');
				}
		...
		if( !$this->input->post('filter') ){
			$count_data = $this->db->query( substr($sql,0,strpos($sql,'LIMIT')) );
			$count_data = $count_data->result();
			$data['pager'] = $this->webspice->pager( count($count_data), $no_of_record, $page_index, $url_prefix.'manage_cultural_activities/page/', 10 );	
		}
		...
		$this->load->view('admin/manage_cultural_activities', $data);
	}

	// add Cultural Activities
	public function add_achievement($data=null) {

		$url_prefix = $this->webspice->settings()->site_url_prefix;
		$this->webspice->user_verify($url_prefix.'login', $url_prefix.'add_achievement');
		$this->webspice->permission_verify('add_achievement');
		if( !isset($data['edit']) ){
			$data['edit'] = array(
				'ACHIEVEMENT_ID'=>null,
				'TITLE'=>null,
		...
		# get input post
		$input = $this->webspice->get_input('achievement_id');
		// dd($input);
		...
		# verify file type
		if( $_FILES['images']['tmp_name'] ){
			$this->webspice->check_file_type(array('jpg','jpeg'), 'images', $data, 'admin/add_achievement');
		}
		...
		if( $input->achievement_id ){
			...
			$sql = "
			UPDATE achievement SET TITLE=?, DETAILS=?, IMAGES=?, UPDATED_BY=?,UPDATED_DATE=?
			WHERE ACHIEVEMENT_ID=?";
			$this->db->query($sql, array($input->title, $input->details, $_FILES['images']['name'], $this->webspice->get_user_id(), $this->webspice->now(), $input->achievement_id));
			$this->webspice->process_image_single('images',$input->achievement_id, 'achievement_full');
			$this->webspice->message_board('Record has been updated!');
			$this->webspice->log_me('product_updated - '.$this->webspice->get_user_id()); # log activities
			$this->webspice->force_redirect($url_prefix.'manage_achievement');
			return false;
		}
		...
		( ?, ?, ?, ?, ?, 7 )";
		$this->db->query($sql, array($input->title, $input->details, $_FILES['images']['name'], $this->webspice->get_user_id(),$this->webspice->now()));
		$this->webspice->process_image_single('images',$this->db->insert_id(), 'achievement_full');
		...
		$this->webspice->message_board('Record inserted successfully!');
		if($this->webspice->permission_verify('manage_achievement',TRUE)){
			$this->webspice->force_redirect($url_prefix . 'manage_achievement');
			return FALSE;
		}
		$this->webspice->force_redirect($url_prefix.'add_achievement');

	}	
	
	public function manage_achievement() {
		$url_prefix = $this->webspice->settings()->site_url_prefix;
		$this->load->database();
		$orderby = 'ORDER BY achievement.ACHIEVEMENT_ID ASC';
		$groupby = null;
		$where = '';
		$page_index = 0;
		$no_of_record = 2000000000;
		...
		switch ($criteria) {
			...
			case 'edit':
				$this->webspice->edit_generator($TableName='achievement', $KeyField='ACHIEVEMENT_ID', $key, $RedirectController='admin_controller', $RedirectFunction='add_achievement', $PermissionName='manage_achievement', $StatusCheck=null, $Log='edit_person');
				return false;
				break;
			case 'update':
				$id = $this->uri->segment(3);
				...
				break;
			case 'inactive':
				$this->webspice->action_executer($TableName='achievement', $KeyField='ACHIEVEMENT_ID', $key, $RedirectURL='manage_achievement', $PermissionName='manage_achievement', $StatusCheck=7, $ChangeStatus=-7, $RemoveCache='achievement', $Log='inactive_person');
				return false;
				break;

			case 'active':
				$this->webspice->action_executer($TableName='achievement', $KeyField='ACHIEVEMENT_ID', $key, $RedirectURL='manage_achievement', $PermissionName='manage_achievement', $StatusCheck=-7, $ChangeStatus=7, $RemoveCache='achievement', $Log='active_person');
				return false;
				break;

			case 'delete':
				$id = $this->webspice->encrypt_decrypt($key, 'decrypt');
				$sql = $this->db->query("DELETE FROM achievement WHERE ACHIEVEMENT_ID='".$id."' LIMIT 1");
				if(!unlink($this->webspice->get_path('cultural_full').$id.'.jpg')) {
					die($this->webspice->get_path('cultural_full').$id.'.jpg');
				}
				if($sql) {
		...
		if( !$this->input->post('filter') ){
			$count_data = $this->db->query( substr($sql,0,strpos($sql,'LIMIT')) );
			$count_data = $count_data->result();
			$data['pager'] = $this->webspice->pager( count($count_data), $no_of_record, $page_index, $url_prefix.'manage_achievement/page/', 10 );	
		}
		...
		$this->load->view('admin/manage_achievement', $data);
	}
