...
class Payment_controller extends CI_Controller {
	...
	public function ie_category($data=null){
		...
		$this->form_validation->set_rules('class_id','class name','required|trim|xss_clean');
		$this->form_validation->set_rules('amount','amount','required|trim|xss_clean');
		$this->form_validation->set_rules('fee_type','fee type','required|xss_clean');
		if( !$this->form_validation->run() ){
		...
		$fee_type = "";
		$fee_type_month = "";
		if(count($input->fee_type) == 2) {
			$fee_type = $input->fee_type[0];
			$fee_type_month = $input->fee_type[1];
		}
		else if(count($input->fee_type) == 1) {
			if($input->fee_type[0]==0) {
				$fee_type = 0;
				$fee_type_month = "";
			}
			else if($input->fee_type[0]==1) {
				$fee_type = "";
				$fee_type_month = 1;
			}
		}
		...
		if( $input->ie_cat_id ){
			...
			$sql = "
			UPDATE ie_payment SET CAT_LIST_ID=?, CLASS_ID=?, FEE_TYPE=?, FEE_TYPE_MONTH=?, AMOUNT=?, UPDATED_BY=?,UPDATED_DATE=?
			WHERE IE_CAT_ID=?";
			$this->db->query($sql, array($input->cat_list_id, $input->class_id, $fee_type, $fee_type_month, $input->amount, $this->webspice->get_user_id(), $this->webspice->now(), $input->ie_cat_id));
		...
		(CAT_LIST_ID, CLASS_ID, FEE_TYPE, FEE_TYPE_MONTH, AMOUNT, YEAR, CREATED_BY, CREATED_DATE, STATUS)
		VALUES
		( ?, ?, ?, ?, ?, ?, ?, ?, 7 )";
		$this->db->query($sql, array($input->cat_list_id, $input->class_id, $fee_type, $fee_type_month, $input->amount, date("Y"), $this->webspice->get_user_id(), $this->webspice->now()));
	...
	public function manage_admission_payment() {
		...
		switch ($criteria) {
			...
			case 'payment':
				...
				$data['services'] = $this->db->query("SELECT * FROM ie_payment WHERE FEE_TYPE=0 AND CLASS_ID='".$class_id."'")->result();
	...
	public function manage_monthly_payment() {
		...
		switch ($criteria) {
			...
			case 'monthly_payment':
				...
				$data['services'] = $this->db->query("SELECT * FROM ie_payment WHERE FEE_TYPE=1 AND CLASS_ID='".$class_id."'")->result();
				$paid_months = array();
				$paid_data = $this->db->query("SELECT DISTINCT MONTH FROM ie_transaction WHERE STUDENT_ID='".$student_id."'")->result();
				foreach($paid_data as $paid_val) {
					$paid_months[] = $paid_val->MONTH;
				}
				$data['paid_months'] = $paid_months;

				$this->load->library('form_validation');
				$this->form_validation->set_rules('month','month','required|xss_clean');
				$this->form_validation->set_rules('discount','discount','xss_clean|numeric');
				if( !$this->form_validation->run() ){
					$this->load->view('admin/payment/monthly_payment', $data);
					return FALSE;
				}

				# get input post
				$input = $this->webspice->get_input('month_id');
				// dd($input);

				$trans_id = $this->webspice->trans_id();
				$discount = $input->discount;
				$monthly_fee = 0;
				foreach($data['services'] as $fees) {
					$monthly_fee += $fees->AMOUNT;
				}
				$sub_total = $monthly_fee * count($input->month);
				if($discount) {
				...
				$sql = "
				INSERT INTO ie_transaction
				(TRANS_ID, IE_CAT_ID, CLASS_ID, SECTION_ID, STUDENT_ID, AMOUNT, DISCOUNT, SUB_TOTAL, MONTH, YEAR, PAYMENT_TYPE, CREATED_BY, CREATED_DATE, STATUS)
				VALUES
				( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 7 )";
				/*foreach($data['services'] as $val) {
					for($i=0; $i<count($input->month); $i++) {
						$this->db->query($sql, array($trans_id, $val->IE_CAT_ID, $class_id, $section_id, $student_id, $val->AMOUNT, $discount, $sub_total, $input->month[$i], $year, $payment_type, $this->webspice->get_user_id(), $this->webspice->now()));
					}
				}*/
				...
				if( !$this->db->insert_id() ){
					$this->webspice->message_board('We could not execute your request. Please tray again later or report to authority.');
					$this->webspice->force_redirect($url_prefix . 'manage_monthly_payment');
					return false;
				}

				$enc_trns_id = $this->webspice->encrypt_decrypt($trans_id, 'encrypt');
				$this->webspice->message_board('Admission payment created successfully');
				$this->webspice->force_redirect($url_prefix . 'manage_monthly_payment/view_payment/'.$enc_trns_id);
				return false;
			break;
			...
			case 'view_payment':
				$data = array();
				$id = $this->webspice->encrypt_decrypt($key, 'decrypt');
				$data['trans_data'] = $this->db->query("SELECT * FROM ie_transaction WHERE TRANS_ID='".$id."' GROUP BY MONTH")->result();
				$data['amount'] = $this->db->query("SELECT SUM(AMOUNT) AS AMOUNT FROM ie_transaction WHERE TRANS_ID='".$id."' GROUP BY MONTH")->row()->AMOUNT;
				// dd($data['amount']);
				...
				$this->load->view('admin/payment/view_monthly_payment', $data);
				return false;
			break;

			case 'monthly_money_recipt':
				$data = array();
				$id = $this->webspice->encrypt_decrypt($key, 'decrypt');
				$data['trans_data'] = $this->db->query("SELECT * FROM ie_transaction WHERE TRANS_ID='".$id."' GROUP BY MONTH")->result();
				$data['amount'] = $this->db->query("SELECT SUM(AMOUNT) AS AMOUNT FROM ie_transaction WHERE TRANS_ID='".$id."' GROUP BY MONTH")->row()->AMOUNT;
				...
				$data['student_id'] = $student_id;
				$data['student_data'] = $this->db->query("SELECT si.NAME, si.BIRTHDAY, si.GENDER, si.RELIGION, si.BLOOD_GROUP, si.AGE, si.PHONE, si.FATHER_NAME, si.MOTHER_NAME, sd.ROLL_NO, s.SECTION_NAME, c.CLASS_NAME FROM student_info AS si INNER JOIN student_data AS sd ON si.STUDENT_ID=sd.STUDENT_ID INNER JOIN section AS s ON s.SECTION_ID=sd.SECTION_ID INNER JOIN class AS c ON c.CLASS_ID=sd.CLASS_ID WHERE sd.STUDENT_DATA_ID='".$student_id."'")->row();
				$this->load->view('admin/payment/print_monthly_money_recipt', $data);
				return false;
			break;
		...
		if( !$this->input->post('filter') ){
			$count_data = $this->db->query( substr($sql,0,strpos($sql,'LIMIT')) );
			$count_data = $count_data->result();
			$data['pager'] = $this->webspice->pager( count($count_data), $no_of_record, $page_index, $url_prefix.'manage_monthly_payment/page/', 10 );
		}
		...
		$this->load->view('admin/payment/manage_monthly_payment', $data);
