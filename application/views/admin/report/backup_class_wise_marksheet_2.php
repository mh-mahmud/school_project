	...
	public function class_wise_marksheet() {
		...
		if($this->input->post("filter")){
			...
			foreach($students as $stupid) {
				$res = $this->db->query("SELECT s.subject_name, m.class_id, m.exam_id, m.section_id, m.mark_obtained, m.student_id FROM SUBJECT AS s LEFT JOIN marks AS m ON s.subject_id = m.subject_id WHERE m.student_id='".$stupid->student_id."' AND m.exam_id='".$exam_id."' AND m.class_id='".$class_id."'")->result();
				$res_data[] = $res;
				dd($res, true);
				echo "<tr>";
