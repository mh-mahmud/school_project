						...
						<div class="achievement-area">
							<div class="single-achievement">
								<div class="home-about-img">
									<?php if( file_exists($this->webspice->get_path('cultural_full').$sglactivities['CULTURAL_ACTIVITIES_ID'].'.jpg') ): ?>
                                        <img src="<?php echo $this->webspice->get_path('cultural').$sglactivities['CULTURAL_ACTIVITIES_ID'].'.jpg'; ?>" width="100%" class="img-responsive img-thumbnail" alt="cultural.jpg">
                                    <?php endif;  ?>
								</div>
								<h4><?php echo $sglactivities['TITLE'] ?></h4>
								<p><kbd>Published :</kbd> <?php
									$date = $sglactivities['CREATED_DATE'];
                                    $month_name = ucfirst(strftime("%d %B %Y", strtotime($date)));
                                    echo $month_name;
                                   ?>
								</p>
								<p><?php echo $sglactivities['DETAILS'] ?></p>
							</div>
						</div>
					</div>
