...
class Webspice{
	...
	function settings(){
		...
		$settings->site_title = 'Fox School Management';
		$settings->domain_name = 'Fox School Management';
		$settings->site_slogan = '';
		$settings->site_url = 'http://cyhs79.edu.bd/';
		$settings->site_url_prefix = '/yusuf_school/';
	...
	function get_path($param=null){
		...
		switch($param){
			...
			case 'person_full':
				...
				break;
			case 'cultural':
				return $url_prefix.'global/custom_files/cultural/';
				break;
			case 'cultural_full':
				return FCPATH.'global/custom_files/cultural/';
				break;
			case 'achievement':
				return $url_prefix.'global/custom_files/achievement/';
				break;
			case 'achievement_full':
				return FCPATH.'global/custom_files/achievement/';
				break;
			case 'slider':
			...
			case 'online_application':
				return $url_prefix.'global/custom_files/online_application/';
				break;
			case 'online_application_full':
				return FCPATH.'global/custom_files/online_application/';
				break;
			default:
	...
	*/
	...
	public function subject_name($id) {
		$CI = &get_instance();
		$CI->load->database();
		$sub_name = $CI->db->query("SELECT SUBJECT_NAME FROM subject WHERE SUBJECT_ID=" . $id)->row()->SUBJECT_NAME;
		return $sub_name;
	}

	public function mark_val_to_points($mark) {
		if($mark >= 80) {
			return 5;
		}
	...
	public function opt_mark_val_to_points($mark) {
		if($mark >= 80) {
			return 3;
		}
		else if( ($mark >= 70) && ($mark <=79) ) {
	...
	public function class_name($id) {
		$CI = &get_instance();
		$CI->load->database();
		$class_name = $CI->db->query("SELECT CLASS_NAME FROM class WHERE CLASS_ID=" . $id)->row()->CLASS_NAME;
		return $class_name;
	}
	...
	public function section_name($id) {
		$CI = &get_instance();
		$CI->load->database();
		$section_name = $CI->db->query("SELECT SECTION_NAME FROM section WHERE SECTION_ID=" . $id)->row()->SECTION_NAME;
		return $section_name;
	}

	public function student_name($id) {
		$CI = &get_instance();
		$CI->load->database();
		$student_name = $CI->db->query("SELECT SI.NAME, SD.STUDENT_DATA_ID, SD.ROLL_NO, SD.CLASS_ID, SD.STUDENT_ID FROM student_info AS SI INNER JOIN student_data AS SD ON SD.STUDENT_ID = SI.STUDENT_ID WHERE SD.STUDENT_DATA_ID='".$id."'")->row()->NAME;
		return $student_name;
	}
	...
	public function trans_id() {
		...
		$trans = $CI->db->query("SELECT TRANS_ID FROM ie_transaction ORDER BY TRANS_ID DESC LIMIT 1")->row();

		if(count($trans)) {
			$trans_id = $trans->TRANS_ID + 1;
		}
		else {
			$trans_id = 1;
		}
	...
	public function category_val($cat_id) {
		$CI = &get_instance();
		$CI->load->database();
		$cat = $CI->db->query("SELECT CATEGORY_NAME, CATEGORY_TYPE FROM category_list WHERE LIST_ID='".$cat_id."'")->row();
		return $cat;
	}
